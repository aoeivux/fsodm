    ...
    for class_name in classes:
        for image_set in sets:
            files = os.listdir(os.path.join(
                datapath, '{}/images'.format(image_set)))
            image_ids = [x.strip('.jpg') for x in files]
            list_file = os.path.join(
                datapath, 'nwpulist/{}_{}.txt'.format(class_name, image_set))
            ...
            with open(list_file, 'w') as out_f:
                for id in image_ids:
                    in_file = os.path.join(
                        datapath, '{}/annotations/{}.txt'.format(image_set, id))
                    out_file = os.path.join(
                        datapath, 'labels_1c/{}/{}.txt'.format(class_name, id))
                    image = os.path.join(
                        datapath, '{}/images/{}.jpg'.format(image_set, id))
